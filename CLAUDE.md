...
- `features/*` 頁面只能呼叫 Store，不可跳過 Store 直接呼叫 Service 或 Axios；Store 呼叫 Service，Service 呼叫共用的 Axios 實例。除了 `services/*` 之外，沒有其他地方能直接呼叫 Axios。即使某個查詢很單純（例如唯讀的下拉選單資料），也要補一個對應的最小 Store（狀態 + fetch action）讓頁面透過它取得資料，而不是讓頁面直接 import Service——參考 `stores/leaveBalance.store.ts`。
...
- 響應式斷點統一為 768px。`DefaultLayout.vue` 在 <768px 時側邊欄不是收合成圖示列，而是變成 **off-canvas 抽屜**：預設隱藏、頂欄漢堡選單（`Fold` icon）觸發滑出，滑出時疊加半透明遮罩，點擊遮罩或任一導覽項目會自動收合（見 `isDrawerOpen`／`closeDrawer`）。含表格的頁面（請假紀錄、待審核、HR 總覽、假別管理）在同一支 `.vue` 內同時渲染桌面表格與手機卡片列表兩份 DOM，用 `@media (max-width: 767px)` 切換 `display`，而非用 JS 條件渲染——避免 resize 時的版面抖動。
...
- Sprint 3（`docs/Sprint-3-Issues.md` 全部 6 個 Issue）已完成：US-005～US-006（Manager 審核流程）的完整垂直切片。新增 Mock API `GET /api/approvals/pending`、`POST /api/approvals/:id/approve`、`POST /api/approvals/:id/reject`（併入 `mocks/handlers/leave.handlers.ts`，直接修改 `MOCK_LEAVE_REQUESTS` 使 Employee 端能立即讀到最新狀態）、Service 層 `services/approvalService.ts`、Store 層 `stores/approval.store.ts`（`pendingRequests`／`processingId` 狀態，`approve`／`reject` 成功後自清單移除該筆），以及 `features/manager/approval/ApprovalListView.vue` 的完整互動實作（點擊查看詳情、核准／駁回二次確認、成功/失敗提示）。目前資料模型無主管－員工對應關係，Manager 看到系統內所有待審核申請，不做團隊範圍篩選；核准也不連動 `leaveBalance` 餘額扣減。
- Sprint 4（`docs/Sprint-4-Issues.md` 全部 6 個 Issue）已完成：US-007～US-008（HR 請假紀錄總覽、假別管理）的完整垂直切片。新增 Mock API `GET /api/hr/leave-records`（併入 `mocks/handlers/leave.handlers.ts`，回傳全體員工的請假紀錄，不受目前登入者限制）、`POST /api/leave-types`／`PUT /api/leave-types/:id`／`DELETE /api/leave-types/:id`（併入 `mocks/handlers/leaveType.handlers.ts`）；Service 層在既有 `services/leaveService.ts` 新增 `getAllLeaveRequests()`，`services/leaveTypeService.ts` 新增 `createLeaveType`／`updateLeaveType`／`deleteLeaveType`；Store 層新增獨立的 `stores/leaveRecord.store.ts`（`records`／`searchKeyword`／`leaveTypeFilter`／`filteredRecords` getter，供 US-007 的搜尋、篩選、排序使用），並擴充既有 `stores/leaveType.store.ts` 加入 CRUD actions 與 `submitting`／`processingId` 狀態；`features/hr/leave-records/LeaveRecordsView.vue`、`features/hr/leave-types/LeaveTypesView.vue` 已完成真實串接（含 Element Plus Form 新增／編輯、`ConfirmDialog` 二次確認刪除）。**架構偏離說明**：`docs/Architecture.md` 第 4 節原將 US-007、US-008 都指派給同一個 `leaveType.store`，但實作時比照 Sprint 3 `approval.store` 獨立於 `leaveRequest.store` 的先例，改為 US-007 用獨立的 `leaveRecord.store`（操作 `LeaveRequest` 領域）、US-008 沿用擴充後的 `leaveType.store`（操作 `LeaveType` 領域），兩者資料形狀不同，不共用同一個 Store。
- 全站視覺重新設計已完成：建立獨立設計系統（`docs/design/`、`src/styles/tokens.css`、`src/components/LeaveRing.vue`），`DefaultLayout`／`AuthLayout` 與全部 7 個頁面皆已套用新版面，並提供桌面版與 768px 斷點下的響應式版面（含側邊欄 off-canvas 抽屜、表格轉卡片列表）。Employee／Manager／HR 各頁皆已是真正串接資料與行為的完整功能頁面，沒有頁面再使用本地靜態陣列或 `ElMessage.info(...)` 尚未串接提示。

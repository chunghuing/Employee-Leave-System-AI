...
- `features/hr/*`（US-007～US-008）尚未排入 Sprint，沒有對應的 Store／Service／MSW handler；但這兩個頁面已套用完整視覺設計，內部用元件本地的靜態陣列取代真實資料，操作按鈕（新增／編輯／刪除假別）點擊後呼叫 `ElMessage.info(...)` 提示「尚未串接」，而不是真的執行動作。等對應 Sprint 排入時，把這些本地陣列替換成真正的 Store／Service 串接即可，版面與樣式不需要重做。`features/manager/approval/ApprovalListView.vue` 已於 Sprint 3 完成真實串接，不再適用此模式。
...
- `features/hr/*`（US-007～US-008，請假紀錄總覽、假別管理）功能本身尚未排入已完成的 Sprint，沒有 Store／Service／MSW handler，頁面內部仍用元件本地的靜態陣列取代真實資料，操作按鈕點擊後僅呼叫 `ElMessage.info(...)` 提示「尚未串接」。
- 全站視覺重新設計已完成：建立獨立設計系統（`docs/design/`、`src/styles/tokens.css`、`src/components/LeaveRing.vue`），`DefaultLayout`／`AuthLayout` 與全部 7 個頁面皆已套用新版面，並提供桌面版與 768px 斷點下的響應式版面（含側邊欄 off-canvas 抽屜、表格轉卡片列表）。HR 兩頁目前仍是套用真實樣式的靜態視覺稿（本地假資料＋操作按鈕提示「尚未串接」），Employee／Manager 各頁則已是真正串接資料與行為的完整功能頁面。
